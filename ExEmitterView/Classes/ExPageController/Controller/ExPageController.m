...
#import "ExPageLoopView.h"

@interface ExPageController ()<UIScrollViewDelegate,ExPageLoopDelegate,UITraitEnvironment>
{
    CGFloat lastContentOffset;
    ExPageNavigationButton *_btnLeft ;
    ExPageNavigationButton *_btnRight;
    CGRect normalUpScRect;
}
//frame数组
@property(nonatomic,strong)NSMutableArray *rectArr;
//头部标题滚动视图
@property(nonatomic,strong)ExPageLoopView *upSc;
//底部全屏滚动视图
@property(nonatomic,strong)ExPageScroller *downSc;
//当前视图
...
@implementation ExPageController
...
- (void)UI{
    ...
    //全屏
    self.downSc = [[ExPageScroller alloc]initWithFrame:self.view.bounds];
    ...
    if (self.parentViewController) {
        ...
        }else if ([self.parentViewController isKindOfClass:[ExPageController class]]) {
            headY = 0;
        }
    }
    ...
    self.upSc = [[ExPageLoopView alloc]initWithFrame:[dic[@(self.param.wMenuPosition)] CGRectValue] param:self.param];
...
- (void)setUpSuspension:(UIViewController*)newVC index:(NSInteger)index{
    ...
    if ([newVC conformsToProtocol:@protocol(ExPageProtocol)]) {
       if ([newVC respondsToSelector:@selector(getTableView)]) {
           UIScrollView *view = [newVC performSelector:@selector(getTableView)];
...
- (void)animalAction:(UIScrollView *)scrollView{
    ...
    if (self.param.wMenuAnimalTitleGradient) {
        ExPageNavigationButton *tempBtn = _btnLeft?:_btnRight;
