...
//固定按钮
@property(nonatomic,strong)ExPageNavigationButton *fixBtn;
@end

@implementation ExPageLoopView
...
- (void)setUp{
    ...
    ExPageNavigationButton *temp = nil;
    for (int i = 0; i<self.param.wTitleArr.count; i++) {
        ExPageNavigationButton *btn = [ExPageNavigationButton buttonWithType:UIButtonTypeCustom];
...
//设置右边固定标题
- (void)setUpFixRightBtn:(ExPageNavigationButton *)temp{
    if (self.param.wMenuFixRightData) {
        ExPageNavigationButton *fixBtn = [ExPageNavigationButton buttonWithType:UIButtonTypeCustom];
...
//设置按钮样式
- (void)setPropertiesWithBtn:(ExPageNavigationButton *)btn withIndex:(int)i withHeightArr:(NSMutableArray*)heightArr withTemp:(ExPageNavigationButton *)temp{
...
- (void)scrollToIndex:(NSInteger)newIndex{
    ...
    ExPageNavigationButton *btn = self.btnArr[newIndex] ;
    //隐藏右上角红点
    [btn hidenBadge];
    //改变背景色
    ...
    //改变标题颜色
    for (ExPageNavigationButton *temp in self.btnArr) {
